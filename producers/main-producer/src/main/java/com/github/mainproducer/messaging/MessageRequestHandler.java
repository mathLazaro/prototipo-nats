...
public class MessageRequestHandler {
    ...
    public ReplyDTO<?> handleJokeRequest(Message message) {
            ...
            return ReplyDTO.success(joke);
    }

    public ReplyDTO<?> handleGreetingRequest(Message message) {
        String request = deserialize(message);
        log.info("Requisição recebida: {}", request);

        if (greetings.stream().anyMatch(greeting -> request.toLowerCase().contains(greeting) || request.equalsIgnoreCase(greeting))) {
            return ReplyDTO.success("Olá tudo bem? Mensagem recebida!");
        }

        return ReplyDTO.success("Mensagem recebida, porém ainda estou aprendendo :/");
